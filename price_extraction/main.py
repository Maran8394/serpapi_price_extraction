...
from price_extraction.extractors.bing_search import BingExtraction
...
@bp.route('/bing-search', methods = ['GET', 'POST']) 
def bing_search():
    if(request.method == 'POST'): 
        data = request.json
        ...
        country = data.get('country')
        use_nlp = data.get('use_nlp', False)  
        query = f"{product} cost in {country}"
        
        # SerpAPI Call
        params = {
            "api_key": "31f11f50a145bc35c52e27812000252c14e1c99902e4d17ba665eb535f7ea2b8",
            "engine": "bing",
            "q": "Rolex Submariner cost in singapore",
            "count": "100",
        }
        search = GoogleSearch(params)
        results = search.get_dict()
        extraction_obj = BingExtraction(query=query,response_data=results,use_nlp=use_nlp)
        extraction_obj.start_extraction()
        sorted_res = extraction_obj.sorted_results()
        sorted_res["use_nlp"] = use_nlp
        sorted_res["status"] = 200
